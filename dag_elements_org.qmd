...
Why to DAG? $\rightarrow$ to sketch out causal assumptions an avoid *causal salad*.
...
But depending on the causal dependencies, variables can interact both *to expose* OR *to mask* the influence of one variable onto another.
...
#| message: false
#| echo: false
models_and_plots <- \(data){
  # ignoring z, x and y are correlated
  cat("Linear Model ignoring z\n-----------------------\n")
  print(summary(lm(y ~ x, data = data)))
  
  # stratified by z, x and y are not associated
  cat("Linear Model stratified by z\n----------------------------\n")
  print(summary(lm(y ~ x + z, data = data)))
...
gg_data_mc <- \(data_mc){
  data_mc |> 
    ...
    geom_point(alpha = .3, show.legend = FALSE) +
    geom_smooth(method = lm, se = FALSE, fullrange = TRUE) +
    geom_smooth(method = lm, se = FALSE, aes(group = 1L), color = "black",
                linetype = 3, fullrange = TRUE) +
    scale_color_manual("z", values = clrs) +
    labs(subtitle = "McElreth Data")
}

plot_data_mc <- \(data_mc){
  attach(data_mc)
...
#| fig.height: 4.
#| message: false
p1 <- models_and_plots(data)
p2 <- gg_data_mc(data_mc)
# plot_data_mc(data_mc)
...
The variable `x` *does* causally influence `y` (through `z`).
But if we *do* include `z` as factor in our model, *we can't infer the effect* of `x` on `y`.
:::
...
The variable `x` *does not* causally influence `y`, their association is spurious.
But if we *do not* include `z` as factor in our model, *we actually find an association between `x` and `y` that might erroneously be interpreted as causal*.
:::

::: {.callout-tip}
## Fork vs. Pipe

*"In data, the fork and the pipe are indistinguishable, but causally they are different!"*<br>
...
*"A restaurant in a good location can make money EVEN IF it has bad food."* and *"With really great food, you can survive even if you are at a bad location."*<br>
...
The variable `x` *does not* causally influence `y`, but both act together to influence `z`.
But if we *do* include `z` as factor in our model, *we actually find an association between `x` and `y` that might erroneously be interpreted as causal*.
:::

::: {.callout-tip}
## Collider Bias

Another tricky aspect of colliders is that they behave differently in terms of *information flow* within the DAG.
Conditioning on a collider *opens* this node, which can create new paths of information flow from `x` to `y`.
This can cause trouble for trying to block all *backdoor paths* between `x` and `y` (paths of associations between `x` and `y` that are not causal).<br>
$\rightarrow$ s. also: @McElreath2020, chapter 5 (*The Many Variables \& The Spurious Waffles*) and chapter 6 (*The Haunted DAG \& The Causal Terror*).
:::

## Bad Controls

Examples of cases where the variable `z` constitutes a *bad control* - that is, it should not be included when inferring the effect of `x` on `y`.
Examples are based on @cinelli2022.
...
- **Bias Amplification:** adding `z` exaggerates the bias introduced by `u` (again by removing variation in `x`)
